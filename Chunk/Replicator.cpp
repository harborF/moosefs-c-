...
static int rep_receive_all_packets(STReplication *r,uint32_t msecto) {
	uint8_t i,l;
...
{
	int i = write(rs->sock,rs->startptr,rs->bytesleft);
	if (i==0) {
		syslog(LOG_NOTICE,"replicator: connection lost");
		return -1;
	}
...
static int rep_send_all_packets(STReplication *r,uint32_t msecto)
{
	uint8_t i,l;
	struct timeval tvb,tv;
	uint32_t msec;
...
static int rep_wait_for_connection(STReplication *r,uint32_t msecto) {
	uint8_t i,l;
	struct timeval tvb,tv;
	uint32_t msec;
	gettimeofday(&tvb,NULL);
...
static void rep_cleanup(STReplication *r) {
	int i;
	if (r->opened) {
		hdd_close(r->chunkid);
	}
...
// get block no
	...
	for (i=0 ; i<srccnt ; i++) {
		...
		uint32_t type = get32bit(&rptr);
		uint32_t size = get32bit(&rptr);
...
// receive status
	...
	for (i=0 ; i<srccnt ; i++) {
		if (r.repsources[i].blocks>0) {
			...
			uint64_t pchid = get64bit(&rptr);
			uint8_t pstatus = get8bit(&rptr);
			if (pchid!=r.repsources[i].chunkid) {
				syslog(LOG_WARNING,"replicator: got wrong answer (read_status:chunkid:%"PRIX64"/%"PRIX64") from (%08"PRIX32":%04"PRIX16")",pchid,r.repsources[i].chunkid,r.repsources[i].ip,r.repsources[i].port);
				rep_cleanup(&r);
				return ERROR_WRONGCHUNKID;
