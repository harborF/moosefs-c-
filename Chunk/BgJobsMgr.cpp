...
typedef struct _jobpool {
	...
	void *jobqueue;
	void *statusqueue;
	STJobEntry* jobhash[JHASHSIZE];
	uint32_t nextjobid;
} STJobPool;

static inline void job_send_status(STJobPool *jp,uint32_t jobid,uint8_t status)
{
...
static inline int job_receive_status(STJobPool *jp,uint32_t *jobid,uint8_t *status) {
	uint32_t qstatus;
	zassert(pthread_mutex_lock(&(jp->pipelock)));
...
static inline uint32_t job_new(STJobPool *jp,uint32_t op,void *args,JobCallback callback,void *extra)
{
	uint32_t jobid = jp->nextjobid;
	uint32_t jhpos = JHASHPOS(jobid);
	STJobEntry *jptr = (STJobEntry*)malloc(sizeof(STJobEntry));
	passert(jptr);
...
void* job_pool_new(uint8_t workers,uint32_t jobs,int *wakeupdesc)
{
    ...
    STJobPool* jp = (STJobPool*)malloc(sizeof(STJobPool));
	passert(jp);
...
uint32_t job_pool_jobs_count(void *jpool) {
	STJobPool* jp = (STJobPool*)jpool;
	return queue_elements(jp->jobqueue);
}

void job_pool_disable_and_change_callback_all(void *jpool,JobCallback callback) {
	STJobPool* jp = (STJobPool*)jpool;
	STJobEntry *jptr;

	zassert(pthread_mutex_lock(&(jp->jobslock)));
	for (uint32_t jhpos = 0 ; jhpos<JHASHSIZE ; jhpos++) {
...
void job_pool_change_callback(void *jpool,uint32_t jobid,JobCallback callback,void *extra)
{
	STJobPool* jp = (STJobPool*)jpool;
	uint32_t jhpos = JHASHPOS(jobid);
	STJobEntry *jptr;
...
uint32_t job_inval(void *jpool,JobCallback callback,void *extra) {
	STJobPool* jp = (STJobPool*)jpool;
	return job_new(jp,OP_INVAL,NULL,callback,extra);
}

uint32_t job_chunkop(void *jpool,JobCallback callback,void *extra,uint64_t chunkid,uint32_t version,uint32_t newversion,uint64_t copychunkid,uint32_t copyversion,uint32_t length) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_op_args *args = (chunk_op_args*)malloc(sizeof(chunk_op_args));
	passert(args);
...
uint32_t job_open(void *jpool,JobCallback callback,void *extra,uint64_t chunkid) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_oc_args *args = (chunk_oc_args*)malloc(sizeof(chunk_oc_args));
	passert(args);
	args->chunkid = chunkid;
	return job_new(jp,OP_OPEN,args,callback,extra);
}

uint32_t job_close(void *jpool,JobCallback callback,void *extra,uint64_t chunkid) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_oc_args *args = (chunk_oc_args*)malloc(sizeof(chunk_oc_args));
	passert(args);
	args->chunkid = chunkid;
...
uint32_t job_read(void *jpool,JobCallback callback,void *extra,uint64_t chunkid,uint32_t version,uint16_t blocknum,uint8_t *buffer,uint32_t offset,uint32_t size,uint8_t *crcbuff) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_rd_args *args = (chunk_rd_args*)malloc(sizeof(chunk_rd_args));
	passert(args);
...
uint32_t job_write(void *jpool,JobCallback callback,void *extra,uint64_t chunkid,uint32_t version,uint16_t blocknum,const uint8_t *buffer,uint32_t offset,uint32_t size,const uint8_t *crcbuff) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_wr_args *args = (chunk_wr_args*)malloc(sizeof(chunk_wr_args));
	passert(args);
...
uint32_t job_replicate(void *jpool,JobCallback callback,void *extra,uint64_t chunkid,uint32_t version,uint8_t srccnt,const uint8_t *srcs) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_rp_args *args;
	uint8_t *ptr = (uint8_t*)malloc(sizeof(chunk_rp_args)+srccnt*18);
	passert(ptr);
...
uint32_t job_replicate_simple(void *jpool,JobCallback callback,void *extra,uint64_t chunkid,uint32_t version,uint32_t ip,uint16_t port) {
	STJobPool* jp = (STJobPool*)jpool;
	chunk_rp_args *args;
	uint8_t *ptr = (uint8_t*)malloc(sizeof(chunk_rp_args)+18);
	passert(ptr);
