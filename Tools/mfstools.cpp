...
void usage(int f) {
	switch (f) {
		...
		case MFSSETEATTR:
			...
			break;
		case MFSDELEATTR:
			fprintf(stderr,"delete objects extra attributes\n\nusage: mfs_deleattr [-nhHr] -f attrname [-f attrname ...] name [name ...]\n");
			print_numberformat_options();
			print_recursive_option();
			...
			break;
		case MFSGETQUOTA:
			fprintf(stderr,"get quota for given directory (directories)\n\nusage: mfs_getquota [-nhH] dirname [dirname ...]\n");
			print_numberformat_options();
			break;
		case MFSSETQUOTA:
			fprintf(stderr,"set quota for given directory (directories)\n\nusage: mfs_setquota [-nhH] [-iI inodes] [-lL length] [-sS size] [-rR realsize] dirname [dirname ...]\n");
			print_numberformat_options();
...
#define SYMLINK(name)	if (symlink(argv[0],name)<0) { \
			...
			SYMLINK("mfs_getgoal")
			SYMLINK("mfs_setgoal")
			SYMLINK("mfs_gettrashtime")
			SYMLINK("mfs_settrashtime")
			SYMLINK("mfs_checkfile")
			SYMLINK("mfs_fileinfo")
			SYMLINK("mfs_appendchunks")
			SYMLINK("mfs_dirinfo")
			SYMLINK("mfs_filerepair")
			SYMLINK("mfs_makesnapshot")
			SYMLINK("mfs_geteattr")
			SYMLINK("mfs_seteattr")
			SYMLINK("mfs_deleattr")
#if VERSHEX>=0x010700
			SYMLINK("mfs_getquota")
			SYMLINK("mfs_setquota")
			SYMLINK("mfs_delquota")
#endif
			// deprecated tools:
			SYMLINK("mfs_rgetgoal")
			SYMLINK("mfs_rsetgoal")
			SYMLINK("mfs_rgettrashtime")
			SYMLINK("mfs_rsettrashtime")
			return 0;
		} else {
			fprintf(stderr,"mfs multi tool\n\nusage:\n\tmfs_tools create - create symlinks (mfs_<toolname> -> %s)\n",argv[0]);
			fprintf(stderr,"\ntools:\n");
			fprintf(stderr,"\tmfs_getgoal\n\tmfs_setgoal\n\tmfs_gettrashtime\n\tmfs_settrashtime\n");
			fprintf(stderr,"\tmfs_checkfile\n\tmfs_fileinfo\n\tmfs_appendchunks\n\tmfs_dirinfo\n\tmfs_filerepair\n");
			fprintf(stderr,"\tmfs_makesnapshot\n");
			fprintf(stderr,"\tmfs_geteattr\n\tmfs_seteattr\n\tmfs_deleattr\n");
#if VERSHEX>=0x010700
			fprintf(stderr,"\tmfs_getquota\n\tmfs_setquota\n\tmfs_delquota\n");
#endif
			fprintf(stderr,"\ndeprecated tools:\n");
			fprintf(stderr,"\tmfs_rgetgoal = mfs_getgoal -r\n");
			fprintf(stderr,"\tmfs_rsetgoal = mfs_setgoal -r\n");
			fprintf(stderr,"\tmfs_rgettrashtime = mfs_gettreshtime -r\n");
			fprintf(stderr,"\tmfs_rsettrashtime = mfs_settreshtime -r\n");
			return 1;
		}
	} else if (CHECKNAME("mfs_getgoal")) {
		f=MFSGETGOAL;
	} else if (CHECKNAME("mfs_rgetgoal")) {
		f=MFSGETGOAL;
		rflag=1;
		fprintf(stderr,"deprecated tool - use \"mfs_getgoal -r\"\n");
	} else if (CHECKNAME("mfs_setgoal")) {
		f=MFSSETGOAL;
	} else if (CHECKNAME("mfs_rsetgoal")) {
		f=MFSSETGOAL;
		rflag=1;
		fprintf(stderr,"deprecated tool - use \"mfs_setgoal -r\"\n");
	} else if (CHECKNAME("mfs_gettrashtime")) {
		f=MFSGETTRASHTIME;
	} else if (CHECKNAME("mfs_rgettrashtime")) {
		f=MFSGETTRASHTIME;
		rflag=1;
		fprintf(stderr,"deprecated tool - use \"mfs_gettrashtime -r\"\n");
	} else if (CHECKNAME("mfs_settrashtime")) {
		f=MFSSETTRASHTIME;
	} else if (CHECKNAME("mfs_rsettrashtime")) {
		f=MFSSETTRASHTIME;
		rflag=1;
		fprintf(stderr,"deprecated tool - use \"mfs_settrashtime -r\"\n");
	} else if (CHECKNAME("mfs_checkfile")) {
		f=MFSCHECKFILE;
	} else if (CHECKNAME("mfs_fileinfo")) {
		f=MFSFILEINFO;
	} else if (CHECKNAME("mfs_appendchunks")) {
		f=MFSAPPENDCHUNKS;
	} else if (CHECKNAME("mfs_dirinfo")) {
		f=MFSDIRINFO;
	} else if (CHECKNAME("mfs_geteattr")) {
		f=MFSGETEATTR;
	} else if (CHECKNAME("mfs_seteattr")) {
		f=MFSSETEATTR;
	} else if (CHECKNAME("mfs_deleattr")) {
		f=MFSDELEATTR;
	} else if (CHECKNAME("mfs_getquota")) {
		f=MFSGETQUOTA;
	} else if (CHECKNAME("mfs_setquota")) {
		f=MFSSETQUOTA;
	} else if (CHECKNAME("mfs_delquota")) {
		f=MFSDELQUOTA;
	} else if (CHECKNAME("mfs_filerepair")) {
		f=MFSFILEREPAIR;
	} else if (CHECKNAME("mfs_makesnapshot")) {
		f=MFSMAKESNAPSHOT;
	} else {
