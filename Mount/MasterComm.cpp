...
static STThRec *s_threadPool=NULL;
static acquired_file *s_pAfHead=NULL;
...
void master_statsptr_init(void) {
	void *s = stats_get_subnode(NULL,"master",0);
...
STThRec* fs_get_my_threc()
{
	...
	if (s_threadPool==NULL) {
		...
		rec->packetid = s_threadPool->packetid+1;
	}
	rec->next = s_threadPool;
	s_threadPool = rec;
	pthread_mutex_unlock(&reclock);
...
STThRec* fs_get_threc_by_id(uint32_t packetid)
{
	pthread_mutex_lock(&reclock);
	for (STThRec *rec = s_threadPool ; rec ; rec=rec->next) {
...
void fs_output_buffer_init(STThRec *rec,uint32_t size)
{
...
void fs_input_buffer_init(STThRec *rec,uint32_t size) 
{
...
uint8_t* fs_createpacket(STThRec *rec,uint32_t cmd,uint32_t size)
{
...
const uint8_t* fs_sendandreceive(STThRec *rec,uint32_t expected_cmd,uint32_t *answer_leng)
{
	static uint8_t notsup = ERROR_ENOTSUP;

	for (uint32_t cnt=0 ; cnt<s_MaxRetries ; cnt++) {
		...
		if (tcpToWrite(s_fd,rec->obuff,rec->odataleng,1000)!=(int32_t)(rec->odataleng))
        {
...
const uint8_t* fs_sendandreceive_any(STThRec *rec,uint32_t *received_cmd,uint32_t *answer_leng)
{
	for (uint32_t cnt=0 ; cnt<s_MaxRetries ; cnt++) 
    {
		pthread_mutex_lock(&fdlock);
...
int fs_connect(uint8_t oninit,struct connect_args_t *cargs)
{
	...
	uint8_t havepassword=(cargs->passworddigest==NULL)?0:1;
	...
	if (tcpNumConnect(s_fd,masterip,masterport)<0) {
		if (oninit) {
	...
	if (havepassword) {
		...
		if (tcpToWrite(s_fd,regbuff,8+65,1000)!=8+65) {
			if (oninit) {
		...
		if (tcpToRead(s_fd,regbuff,8,1000)!=8) {
			if (oninit) {
		...
		if (tcpToRead(s_fd,regbuff,32,1000)!=32) {
			if (oninit) {
				fprintf(stderr,"error receiving data from mfsmaster\n");
			} else {
				syslog(LOG_WARNING,"error receiving data from mfsmaster");
	...
	if (tcpToWrite(s_fd,regbuff,8+64+(cargs->meta?9:13)+ileng+pleng+(havepassword?16:0),1000)!=(int32_t)(8+64+(cargs->meta?9:13)+ileng+pleng+(havepassword?16:0))) {
		if (oninit) {
	...
	if (tcpToRead(s_fd,regbuff,i,1000)!=(int32_t)i) {
		if (oninit) {
			fprintf(stderr,"error receiving data from mfsmaster: %s\n",strerr(errno));
		} else {
			syslog(LOG_WARNING,"error receiving data from mfsmaster: %s",strerr(errno));
...
void fs_reconnect() {
	...
	if (tcpToWrite(s_fd,regbuff,8+64+9,1000)!=8+64+9) {
...
void fs_close_session(void) {
	...
	put32bit(&wptr,sessionid);
	if (tcpToWrite(s_fd,regbuff,8+64+5,1000)!=8+64+5) {
...
void* fs_nop_thread(void *arg) {
	...
	for (;;) {
		...
		if (disconnect==0 && s_fd>=0) {
			if (lastwrite+2<now) {	// NOP
				ptr = hdr;
				put32bit(&ptr,ANTOAN_NOP);
				put32bit(&ptr,4);
				put32bit(&ptr,0);
				if (tcpToWrite(s_fd,hdr,12,1000)!=12) {
					disconnect=1;
				} else {
			...
			if (inodeswritecnt==0) {	// HELD INODES
				...
				//inodesleng=24;
				inodesleng=8;
				for (afptr=s_pAfHead ; afptr ; afptr=afptr->next) {
				...
				if (tcpToWrite(s_fd,inodespacket,inodesleng,1000)!=inodesleng) {
					disconnect=1;
				} else {
...
void* fs_receive_thread(void *arg)
{
	...
	for (;;) {
		...
		if (disconnect) {
			...
			s_fd=-1;
			disconnect=0;
			// send to any STThRec status error and unlock them
			pthread_mutex_lock(&reclock);
			for (rec=s_threadPool ; rec ; rec=rec->next) {
				pthread_mutex_lock(&(rec->mutex));
				if (rec->sent) {
		...
		pthread_mutex_unlock(&fdlock);
		r = tcpToRead(s_fd,hdr,12,RECEIVE_TIMEOUT*1000);	// read timeout - 4 seconds
        ...
        if (size>0) {
			r = tcpToRead(s_fd,rec->ibuff,size,1000);
...
{
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_STATFS,0);
	if (wptr==NULL) {
		*totalspace = 0;
		*availspace = 0;
		*trashspace = 0;
    ...
    uint32_t i;
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_STATFS,&i);
	if (rptr==NULL || i!=36) {
		*totalspace = 0;
		*availspace = 0;
		*trashspace = 0;
...
uint8_t fs_access(uint32_t inode,uint32_t uid,uint32_t gid,uint8_t modemask) {
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_ACCESS,13);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
    ...
    uint8_t ret;
	uint32_t i;
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_ACCESS,&i);
	if (!rptr || i!=1) {
		ret = ERROR_IO;
	} else {
		ret = rptr[0];
...
{
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_LOOKUP,13+nleng);
	if (wptr==NULL) {
		return ERROR_IO;
	}
    ...
    uint8_t ret;
    uint32_t i;
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_LOOKUP,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
{
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETATTR,12);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
    ...
    uint32_t i;
    uint8_t ret;
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_GETATTR,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_setattr(uint32_t inode,uint32_t uid,uint32_t gid,uint8_t setmask,uint16_t attrmode,uint32_t attruid,uint32_t attrgid,uint32_t attratime,uint32_t attrmtime,uint8_t sugidclearmode,uint8_t attr[35]) {
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_SETATTR,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_truncate(uint32_t inode,uint8_t opened,uint32_t uid,uint32_t gid,uint64_t attrlength,uint8_t attr[35])
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_TRUNCATE,21);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_TRUNCATE,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_readlink(uint32_t inode,const uint8_t **path)
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_READLINK,4);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);

	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_READLINK,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_symlink(uint32_t parent,uint8_t nleng,const uint8_t *name,const uint8_t *path,uint32_t uid,uint32_t gid,uint32_t *inode,uint8_t attr[35]) {
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint32_t t32 = strlen((const char *)path)+1;
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_SYMLINK,t32+nleng+17);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,parent);
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_SYMLINK,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_mknod(uint32_t parent,uint8_t nleng,const uint8_t *name,uint8_t type,uint16_t mode,uint32_t uid,uint32_t gid,uint32_t rdev,uint32_t *inode,uint8_t attr[35]) 
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_MKNOD,20+nleng);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,parent);
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_MKNOD,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_mkdir(uint32_t parent,uint8_t nleng,const uint8_t *name,uint16_t mode,uint32_t uid,uint32_t gid,uint8_t copysgid,uint32_t *inode,uint8_t attr[35])
{
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_MKDIR,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_rmdir(uint32_t parent,uint8_t nleng,const uint8_t *name,uint32_t uid,uint32_t gid)
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_RMDIR,13+nleng);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,parent);
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_RMDIR,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_rename(uint32_t parent_src,uint8_t nleng_src,const uint8_t *name_src,uint32_t parent_dst,uint8_t nleng_dst,const uint8_t *name_dst,uint32_t uid,uint32_t gid,uint32_t *inode,uint8_t attr[35])
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_RENAME,18+nleng_src+nleng_dst);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_RENAME,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_link(uint32_t inode_src,uint32_t parent_dst,uint8_t nleng_dst,const uint8_t *name_dst,uint32_t uid,uint32_t gid,uint32_t *inode,uint8_t attr[35]) {
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_LINK,17+nleng_dst);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_LINK,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_getdir(uint32_t inode,uint32_t uid,uint32_t gid,const uint8_t **dbuff,uint32_t *dbuffsize) 
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETDIR,12);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_GETDIR,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_getdir_plus(uint32_t inode,uint32_t uid,uint32_t gid,uint8_t addtocache,const uint8_t **dbuff,uint32_t *dbuffsize)
{
	...
	uint8_t flags;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETDIR,13);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
...
uint8_t fs_opencheck(uint32_t inode,uint32_t uid,uint32_t gid,uint8_t flags,uint8_t attr[35])
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_OPEN,13);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	fs_inc_acnt(inode);
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_OPEN,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		if (attr) {
...
uint8_t fs_readchunk(uint32_t inode,uint32_t indx,uint64_t *length,uint64_t *chunkid,uint32_t *version,const uint8_t **csdata,uint32_t *csdatasize)
{
	...
	*csdata=NULL;
	*csdatasize=0;
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_READ_CHUNK,8);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_READ_CHUNK,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_writechunk(uint32_t inode,uint32_t indx,uint64_t *length,uint64_t *chunkid,uint32_t *version,const uint8_t **csdata,uint32_t *csdatasize)
{
	...
	*csdata=NULL;
	*csdatasize=0;
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_WRITE_CHUNK,8);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_WRITE_CHUNK,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_writeend(uint64_t chunkid, uint32_t inode, uint64_t length)
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_WRITE_CHUNK_END,20);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_WRITE_CHUNK_END,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_getreserved(const uint8_t **dbuff,uint32_t *dbuffsize)
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETRESERVED,0);
	if (wptr==NULL) {
		return ERROR_IO;
	}

	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_GETRESERVED,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_gettrash(const uint8_t **dbuff,uint32_t *dbuffsize) 
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETTRASH,0);
	if (wptr==NULL) {
		return ERROR_IO;
	}

	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_GETTRASH,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_gettrashpath(uint32_t inode,const uint8_t **path)
{
	...
	uint8_t ret;
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETTRASHPATH,4);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);

	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_GETTRASHPATH,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_settrashpath(uint32_t inode,const uint8_t *path)
{
	...
	STThRec *rec = fs_get_my_threc();
	uint32_t t32 = strlen((const char *)path)+1;
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_SETTRASHPATH,t32+8);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
    ...
    const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_SETTRASHPATH,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
{
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_UNDEL,4);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	put32bit(&wptr,inode);
    ...
    uint32_t i;
    uint8_t ret;
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_UNDEL,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_getxattr(uint32_t inode,uint8_t opened,uint32_t uid,uint32_t gid,uint8_t nleng,const uint8_t *name,uint8_t mode,const uint8_t **vbuff,uint32_t *vleng) {
	...
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETXATTR,15+nleng);
	if (wptr==NULL) {
		return ERROR_IO;
	}
	...
	const uint8_t *rptr = fs_sendandreceive(rec,MATOCL_FUSE_GETXATTR,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else if (i==1) {
		ret = rptr[0];
...
uint8_t fs_listxattr(uint32_t inode,uint8_t opened,uint32_t uid,uint32_t gid,uint8_t mode,const uint8_t **dbuff,uint32_t *dleng)
{
	...
	uint8_t *wptr = fs_createpacket(rec,CLTOMA_FUSE_GETXATTR,15);
	if (wptr==NULL) {
		return ERROR_IO;
	}
...
{
	STThRec *rec = fs_get_my_threc();
	uint8_t *wptr = fs_createpacket(rec,qcmd,queryleng);
	if (wptr==NULL) {
		return ERROR_IO;
	}
    ...
    uint32_t i;
    uint8_t ret;
	const uint8_t *rptr = fs_sendandreceive_any(rec,acmd,&i);
	if (rptr==NULL) {
		ret = ERROR_IO;
	} else {
		if (*answerleng<i) {
