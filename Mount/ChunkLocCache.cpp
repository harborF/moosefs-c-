...
static STHashBucket *s_ChunkLocHash = NULL;
...
{
	void *s = stats_get_subnode(NULL,"chunkloc_cache_test",0);
...
void chunkloc_cache_insert(uint32_t inode,uint32_t pos,uint64_t chunkid,uint32_t chunkversion,uint8_t csdatasize,const uint8_t *csdata)
{
	...
	STHashBucket *hb,*fhb = NULL;
	uint8_t h,i,fi = 0;
	uint32_t now = time(NULL);
	uint32_t mints = UINT32_MAX;

	pthread_mutex_lock(&clcachelock);
	for (h=0 ; h<HASH_FUNCTIONS ; h++) 
    {
		hb = s_ChunkLocHash + ((inode*primes[h]+pos*primes[HASH_FUNCTIONS-1-h])%HASH_BUCKETS);
		for (i=0 ; i<HASH_BUCKET_SIZE ; i++) 
        {
			if (hb->inode[i]==inode && hb->pos[i]==pos)
            {
				if (hb->chunkid[i]!=chunkid 
                    || hb->chunkversion[i]!=chunkversion 
                    || hb->csdatasize[i]!=csdatasize 
                    || chunkloc_compare(hb->csdata[i],csdata,csdatasize)==0)
                {
...
{
	s_ChunkLocHash = (STHashBucket *)malloc(sizeof(STHashBucket)*HASH_BUCKETS);
	memset(s_ChunkLocHash,0,sizeof(STHashBucket)*HASH_BUCKETS);
...
void chunkloc_cache_term(void)
{
	...
	pthread_mutex_lock(&clcachelock);
	for (uint32_t hi=0 ; hi<HASH_BUCKETS ; hi++) {
		hb = s_ChunkLocHash + hi;
		for (i=0 ; i<HASH_BUCKET_SIZE ; i++) {
			if (hb->csdata[i]) {
				free(hb->csdata[i]);
			}
