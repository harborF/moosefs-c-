...
static STHashBucket *s_SymlinkHash = NULL;
...
static inline void symlink_cache_statsptr_init(void) {
	void *s = stats_get_subnode(NULL,"symlink_cache",0);
...
void symlink_cache_insert(uint32_t inode,const uint8_t *path) {
	...
	for (h=0 ; h<HASH_FUNCTIONS ; h++) {
		hb = s_SymlinkHash + ((inode*primes[h])%HASH_BUCKETS);
...
void symlink_cache_init(void) {
	s_SymlinkHash = (STHashBucket*)malloc(sizeof(STHashBucket)*HASH_BUCKETS);
	memset(s_SymlinkHash,0,sizeof(STHashBucket)*HASH_BUCKETS);
