...
static STFhEntry *s_pFhHead=NULL;
...
{
	pthread_mutex_lock(&opbufflock);
	STFhEntry *fhptr = (STFhEntry*)malloc(sizeof(STFhEntry));
	fhptr->fh = nextfh++;
	fhptr->refcount = 1;
	if (hflag) {
		if (writepos<MAXHISTORYSIZE) {
			...
			uint32_t bpos = fhptr->readpos%OPBUFFSIZE;
...
void oplog_getdata(unsigned long fh,uint8_t **buff,uint32_t *leng,uint32_t maxleng)
{
	...
	pthread_mutex_lock(&opbufflock);
	for (fhptr=s_pFhHead ; fhptr && fhptr->fh != fh ; fhptr=fhptr->next ) {	}
