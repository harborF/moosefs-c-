...
uint8_t exports_check(uint32_t ip,uint32_t version,uint8_t meta,const uint8_t *path,const uint8_t rndcode[32],const uint8_t passcode[16],uint8_t *sesflags,
                      uint32_t *rootuid,uint32_t *rootgid,
                      uint32_t *mapalluid,uint32_t *mapallgid,
                      uint8_t *mingoal,uint8_t *maxgoal,
...
int exports_parseline(char *line,uint32_t lineno,exports *arec) {
	...
	*p=0;
	...
	if (*p==0) {
		...
		arec->pleng = pleng;
		if (pleng>0) {
			arec->path = (uint8_t*)malloc(pleng+1);
			passert(arec->path);
...
int exports_init(void) {
	...
	CServerCore::getInstance()->reload_register(exports_reload);
	CServerCore::getInstance()->destruct_register(exports_term);
	return 0;
}
